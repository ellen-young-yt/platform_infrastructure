...
class TestInfrastructureStatus:
    ...
    def test_status_initialization(self, test_environment):
        ...
        assert status.tfvars_file == Path(__file__).parent.parent.parent / f"{test_environment}.tfvars"
    ...
    def test_get_terraform_state_success(self, mock_run_command, test_environment, tmp_path):
        ...
        mock_run_command.return_value = (True, "module.vpc.aws_vpc.main\nmodule.s3.aws_s3_bucket.data_lake", "")
    ...
    @patch('status.InfrastructureStatus.run_command')
    def test_get_terraform_state_command_failure(self, mock_run_command, test_environment, tmp_path):
    ...
    def test_show_resource_counts_empty(self, test_environment):
        ...
        warning_calls = [call for call in mock_print.call_args_list if 'No resources found' in str(call)]
    ...
    @patch('status.InfrastructureStatus.run_command')
    def test_get_terraform_outputs_success(self, mock_run_command, test_environment, sample_terraform_outputs):
        """Test successful terraform outputs retrieval."""
        status = InfrastructureStatus(test_environment)
        
        mock_run_command.return_value = (True, json.dumps(sample_terraform_outputs), "")
    ...
    def test_show_access_information_empty(self, test_environment):
        ...
        warning_calls = [call for call in mock_print.call_args_list if 'No outputs available' in str(call)]
        assert len(warning_calls) > 0
