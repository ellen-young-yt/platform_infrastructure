...
class TestInfrastructureState:
        ...
        try:
            ...
            for statement in statements:
                ...
                if isinstance(principals, dict) and 'AWS' in principals:
                    ...
                    if any('snowflake' in principal.lower() or 'QHAQNPB-NO48574' in principal for principal in aws_principals):
    ...
    def test_vpc_exists_and_configured(self, environment, test_project_name):
        ...
        try:
            ...
            assert cidr_block.startswith('10.'), f"VPC CIDR should be in private range, got {cidr_block}"
    ...
    def test_subnets_exist_in_multiple_azs(self, environment, test_project_name):
        ...
        try:
            ...
            assert len(availability_zones) >= 2, f"Subnets should be in multiple AZs, found: {availability_zones}"
    ...
    def test_ecs_clusters_exist(self, environment, test_project_name):
        ...
        try:
            for cluster_name in expected_clusters:
                ...
                cluster = clusters[0]
                assert cluster['status'] == 'ACTIVE', f"Cluster {cluster_name} should be ACTIVE, got {cluster['status']}"
    ...
    def test_terraform_state_accessibility(self):
        ...
        try:
            ...
            expected_types = {'aws_vpc', 'aws_s3_bucket', 'aws_secretsmanager_secret'}
            
            found_types = expected_types.intersection(resource_types)
            assert len(found_types) > 0, f"Expected resource types {expected_types}, found {resource_types}"
